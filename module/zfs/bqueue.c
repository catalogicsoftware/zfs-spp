...
/* Flag indicating the bqueue is closed; all enqueues will fail. */
#define	BQUEUE_F_CLOSED		(1U << 0)
...
void
bqueue_close(bqueue_t *q)
{

	mutex_enter(&q->bq_lock);
	q->bq_flags |= BQUEUE_F_CLOSED;
...
boolean_t
bqueue_is_closed(bqueue_t *q)
{

	return q->bq_flags & BQUEUE_F_CLOSED;
}

/*
 * Take the first element off of q.  If there are no elements on the queue, wait
 * until one is put there, or the queue is closed.  Return the removed element,
 * or NULL upon closure once the queue is empty.
 */
void *
